...
# on-off state with attention mechanism
class S2S_on_attention(nn.Module):
    def __init__(self, window_len, out_len, filters, kernal, units):
        super(S2S_on_attention, self).__init__()
        # regression network
        self.conv1_p = nn.Conv1d(1, 30, 13, padding=6)  # 10
        #         self.conv1_p = nn.Conv1d(1, filters, kernal, padding=7)
        self.conv2_p = nn.Conv1d(30, 30, 11, padding=5)  # 8
        #         self.conv1_p = nn.Conv1d(filters, filters, kernal, padding=7)
        self.conv3_p = nn.Conv1d(30, 40, 9, padding=4)  # 6
        #         self.conv1_p = nn.Conv1d(filters, filters, kernal, padding=7)
        self.conv4_p = nn.Conv1d(40, filters, 7, padding=3)
        #         self.conv1_p = nn.Conv1d(filters, filters, kernal, padding=7)
        # [256, 50, 400]
        self.lstm5_p = nn.LSTM(window_len, units, bidirectional=True)
        # [256, 50, 256]
        self.atten_p = AttentionLayer(units, filters)
        self.fc1_p = nn.Linear(units * 2 * filters, units)
        #         self.fc1_p = nn.Linear(256, 1024)
        self.fc2_p = nn.Linear(units, out_len)
    ...
    def forward(self, x):
        ...
        x = F.relu(self.conv4_p(x))
        x, (h_T, c_T) = self.lstm5_p(x)
        x = x.flatten(-2, -1)
        x, weights = self.atten_p(x)
        #         print('x.shape', x.shape)
        #         x = x.flatten(-1, 0)
        x = F.relu(self.fc1_p(x))
        x = self.fc2_p(x)
...
class S2S_state_attention(nn.Module):
    def __init__(self, window_len, out_len, state_num, units, filters):
        super(S2S_state_attention, self).__init__()
        self.conv1_p = nn.Conv1d(1, filters, 13, padding=6)
        self.conv2_p = nn.Conv1d(filters, filters, 11, padding=5)
        self.conv3_p = nn.Conv1d(filters, filters, 7, padding=3)
        self.conv4_p = nn.Conv1d(filters, filters, 5, padding=2)
        self.lstm5_p = nn.LSTM(window_len, units, bidirectional=True)
        # [batch, filters, units*2]
        self.atten_p = ExternalAttention(d_model=units * 2, S=128)
        #         self.atten_p = simp(d_model=units*2, h=4)
        # [batch, filters, units*2]
        # self.atten_p = AttentionLayer(units, filters)
        self.fc1_p = nn.Linear(units * 2 * filters, units)
        self.fc2_p = nn.Linear(units, out_len * state_num)

        # input=[batch, 1, window_len]
        self.conv1_s = nn.Conv1d(1, 30, 13, padding=6)
        self.conv2_s = nn.Conv1d(30, 30, 11, padding=5)
        self.conv3_s = nn.Conv1d(30, 40, 7, padding=3)
        self.conv4_s = nn.Conv1d(40, 50, 5, padding=2)
...
class S2S_state_attention_CNN(nn.Module):
    def __init__(self, window_len, out_len, state_num, units, filters):
        super(S2S_state_attention_CNN, self).__init__()
        # [batch, 1, window_len]
        self.conv1_p = nn.Conv1d(1, filters, 13, padding=6)
        self.conv2_p = nn.Conv1d(filters, filters, 11, padding=5)
        self.conv3_p = nn.Conv1d(filters, filters, 7, padding=3)
        self.conv4_p = nn.Conv1d(filters, filters, 5, padding=2)
        # [batch, 60, window_len]
        #         self.atten1_p = multiEA(d_model=window_len, S=64, H=8)
#         self.atten1_p = ExternalAttention(d_model=window_len, S=64)
#         self.atten1_p = simp(d_model=window_len, h=16)
        self.atten1_p = SA(d_model=window_len, d_k=window_len, d_v=window_len, h=16)
#         self.atten2_p = simp(d_model=window_len, h=16)
#         self.atten3_p = simp(d_model=window_len, h=16)
        # [batch, 60, window_len]
        self.fc1_p = nn.Linear(window_len * filters, units)
        self.fc2_p = nn.Linear(units, out_len * state_num)

        self.conv1_s = nn.Conv1d(1, filters, 13, padding=6)
        self.conv2_s = nn.Conv1d(filters, filters, 11, padding=5)
        self.conv3_s = nn.Conv1d(filters, filters, 7, padding=3)
        self.conv4_s = nn.Conv1d(filters, filters, 5, padding=2)
        self.conv5_s = nn.Conv1d(filters, filters, 5, padding=2)
        self.fc1_s = nn.Linear(filters * window_len, units)
        self.fc2_s = nn.Linear(units, out_len * state_num)
    ...
    def forward(self, x):
        ...
        x = F.relu(self.conv3_p(x))
        x = F.relu(self.conv4_p(x))
        x = self.atten1_p(x, x, x)
#         x= torch.cat((self.atten1_p(x, x, x), self.atten2_p(x, x, x), self.atten3_p(x, x, x)), 1)
        x = x.flatten(-2, -1)
        x = F.relu(self.fc1_p(x))
        x = self.fc2_p(x)
...
class S2S_on_attention_CNN(nn.Module):
    def __init__(self, window_len, out_len, units, filters):
        super(S2S_on_attention_CNN, self).__init__()
        self.conv1_p = nn.Conv1d(1, filters, 13, padding=6)
        self.conv2_p = nn.Conv1d(filters, filters, 11, padding=5)
        self.conv3_p = nn.Conv1d(filters, filters, 7, padding=3)
        self.conv4_p = nn.Conv1d(filters, filters, 5, padding=2)
        self.conv5_p = nn.Conv1d(filters, filters, 5, padding=2)
        # [batch, filters, window_len]
        self.atten_p = ExternalAttention(d_model=window_len, S=256)
        #         self.atten_p = simp(d_model=window_len*2, h=2)
        # [batch, filters, window_len]
        self.fc1_p = nn.Linear(window_len * filters, units)
        self.fc2_p = nn.Linear(units, out_len)
